...
> [!NOTE] > `()` is returned when a function doesn't return an expression (quite similar to `void` or `None`), example below
...
> [!NOTE]
> Note: `if` is not a statment, it's an **expression**. That means it always returns something.
